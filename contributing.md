...
cd C:\FORMULA\Code\z_workspace\vcu
...
```powershell
cd C:\
mkdir z_workspace-clean
cd z_workspace-clean
git clone https://github.com/VT-Motorsports/z_workspace .
west init -l .
```
...
#### "Board 'vcu_stm32' not found"

**Symptoms:**
```
CMake Error: No board named 'vcu_stm32' found
```
...
```powershell
cd vcu
dir boards\arm\vcu_stm32

# Should contain:
# vcu_stm32.dts           (devicetree)
# vcu_stm32_defconfig     (Kconfig defaults)
# vcu_stm32.yaml          (board metadata)
...
Verify your board definition has correct flash size in `vcu_stm32.dts`:
```dts
...
Add to `vcu/boards/arm/vcu_stm32/board.cmake`:
```cmake
